...
public class Gambit {
	...
	public static void main(String[] args) {
		...
		input.nextLine();// ψαχνουμε αν ειναι η σωστη εντολη
		// Easy e = new Easy();
		// Hard h = new Hard();

		int max = 0;
		while (max < 3) { // ελεχγω αν ειναι η σωστη λουπα
			max = setRound(max);
			System.out.println("Round " + max); // να αλλαζει οταν ενας παικτης αλλαζει γυρο οταν φτασει στο 24
			...
			for (int i = 0; i < num; i++) {
				...
				if (arr[i].getPos() == 24) {
					...
					Bank.salary(i, arr);
					System.out.println("Είσαι στην αφετηρία" + arr[i].getCharacter());
				} else {
					if (arr[i].getPos() > 24) {
						...
						arr[i].setRound(arr[i].getRound() + 1);
						Bank.salary(i, arr);
						System.out.println("Πέρασες στην αφετηρία" + arr[i].getCharacter());
					}
					if (arr[i].getRound() == 1) {
