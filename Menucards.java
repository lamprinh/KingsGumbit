...
public class Menucards {
 public static void Menu1 (int amount, Player []arr, int i, int j , int k, int l) {
 ...
 if (Bank.King_area[l_pos] == 1 && p != p1) {
		...
		int w2= arr[0].getWallet() +500;
		p.setWallet(w1);
		arr[0].setWallet(w2);
} else if ( Bank.King_area[l_pos] == 2 && p != p2) {
		int w1= p.getWallet() - 500;
		int w3= arr[1].getWallet() +500;
		p.setWallet(w1);
		arr[1].setWallet(w3);
} else if (Bank.King_area[l_pos] == 3 && p != p3) {
		int w1= p.getWallet() - 500;
		int w4= arr[2].getWallet() +500;
		p.setWallet(w1);
		arr[2].setWallet(w4);

} else if (Bank.King_area[l_pos] == 4 && p != p4) {
		int w1= p.getWallet() - 500;
		int w5= arr[3].getWallet() +500;
		p.setWallet(w1);
		arr[3].setWallet(w5);
}
return flag;
}
public static boolean Menu4 (int amount, Player []arr, int j) {
...
public static boolean Menu5 (int amount, Player []arr, int j , int k) {
	int counterp = 0;
	int counterp1 = 0;
	for (int i=0; i < Bank.King_area.length; i++) {
		if (Bank.King_area[i] == arr[j]) {
...
public static void Menu6 (Player []arr, int i , int j, int k, int l, int amount) {
	Player p = arr[i];
	Player p1 = arr[j];
	Player p2 = arr[k];
	Player p3 = arr[l];
	int wallet = p.getWallet() - 3 * amount;
	int wallet1 = p1.getWallet() + amount;
	int wallet2 = p2.getWallet() + amount;
	int wallet3 = p3.getWallet() + amount;
	p.setWallet(wallet);
	p1.setWallet(wallet1);
	p2.setWallet(wallet2);
	p3.setWallet(wallet3);
}
public static void Menu12 (Player []arr, int i , int j , int k , int l, int amount, int sum) {
    Player p = arr[i];
	Player p1 = arr[j];
	Player p2 = arr[k];
	Player p3 = arr[l];
